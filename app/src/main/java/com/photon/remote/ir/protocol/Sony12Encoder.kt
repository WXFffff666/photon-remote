 ...
 *  - 整帧补零到 45000 µs
 *  - 位序：**整体 LSB 先发**（12 位 = 5 位地址 + 7 位命令，编码器按整体数值处理）
 *  - 输入 3 位 hex → 12 bit
 *  - 重复行为：完整帧重发（PressKind 不影响波形），无协议级重复间隔覆盖
 *
 * 无状态编码器，使用单例 object。
 */
object Sony12Encoder : IrProtocolEncoder {

    override val protocol: ProtocolType = ProtocolType.SONY12
    ...
    const val BIT_COUNT = 12             // 数据位宽

    /**
     * 编码 SONY 12 位波形。
     *
     * @param hex hex 码串：1-3 位十六进制，左补 0 到 12 位；非法字符 / 超过 3 位抛 [IllegalArgumentException]
     * @param press 任意（SONY 完整帧重发，PressKind 不影响波形）
     */
    override fun encode(hex: String, press: PressKind): IRPattern {
        val normalized = hex.trim().uppercase().padStart(3, '0')
        require(normalized.length <= 3) { "SONY12 码最多 3 位十六进制: $hex" }
        ...
        for (b in bits) {                             // 12 位（删尾：无帧尾 mark）
            list += BIT_MARK
